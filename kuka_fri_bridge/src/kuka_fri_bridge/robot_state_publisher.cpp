...
Robot_state_publisher::Robot_state_publisher(ros::NodeHandle& nh, const robot::LWRRobot &lWRobot):
    mLWRRobot(lWRobot)
{

    std::string topicName;
    ///- Publish Joint State Message -///
    topicName = "KUKA/joint_states";
    jointStatePublisher = nh.advertise<sensor_msgs::JointState>(topicName,3);

    ///- Publish Joint State Impedance Message -///
    topicName = "KUKA/joint_imp_states";
...
/*
   ...
   /* eeFT.Resize(6);
    ...
    jointStateMsg.position.resize(KUKA_DOF);
    jointStateMsg.velocity.resize(KUKA_DOF);
    jointStateMsg.effort.resize(KUKA_DOF);
    jointStateMsg.name.resize(KUKA_DOF);

    ///--- Initializing Joint State Impedance Message ---///
    jointStateImpedanceMsg.position.resize(KUKA_DOF);
    jointStateImpedanceMsg.velocity.resize(KUKA_DOF);
    jointStateImpedanceMsg.effort.resize(KUKA_DOF);
    jointStateImpedanceMsg.stiffness.resize(KUKA_DOF);
    jointStateImpedanceMsg.name.resize(KUKA_DOF);
    ...
    */
    ...
    for(int i=0; i<KUKA_DOF; ++i) {
        ...
        buff = "dof_" + boost::lexical_cast<std::string>(i);
        jointStateMsg.name[i]          = buff;
        jointStateImpedanceMsg.name[i] = buff;
    }
...
void Robot_state_publisher::update(){
...
*/
   //  Eigen::Quaternion<double> rot_quat(rot_Eigen);
     ...
     arma::colvec curr_stiff;
     curr_stiff.resize(KUKA_DOF);
     curr_stiff = mLWRRobot.GetJointStiffness();
